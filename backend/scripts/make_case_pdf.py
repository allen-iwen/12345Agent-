...
def build(d: dict) -> Document:
    ...
    # ---------- 七、桌面实操截图 ----------
    h1(doc, "七、桌面实操截图")
    para(doc, "以下四张是 AI 工具在桌面上的实际运行画面，按时间顺序记录了本次补充材料的完整过程，"
              "可直接看到本项目的会话、工具调用卡片、所用模型、权限模式与运行计数。")
    for name, cap in (
        ("desktop_01_DeepSeekHarness.png",
         "图 5　AI 工具的桌面窗口：左侧工作区列表（本项目「12345工单热线」），"
         "中间为本项目会话与工具调用卡片，底部为模型、权限模式与运行计数。"),
        ("desktop_02_工作状态_取证与验证.png",
         "图 6　同一窗口的后续工作状态：会话中给出取证方式建议，"
         "下方是三个交付物卡片（实操案例 PDF、桌面截图、复赛提交包）。"),
        ("desktop_03_工作状态_验证与材料生成.png",
         "图 7　继续推进中的工作状态：验证与材料生成阶段的会话记录。"),
        ("desktop_04_实时工作状态_验证与取证.png",
         "图 8　实时工作画面：可见推理过程与 Pwsh 工具调用卡片"
         "（卡片描述即当时正在执行的动作），底部为运行计数与缓存命中率。"),
    ):
        picture(doc, SHOTS / name, cap, max_height_cm=16.5)
    para(doc, "取证方式与取舍：截图由 scripts/capture_desktop_shots.ps1 取得——"
              "用 Win32 PrintWindow 让目标窗口渲染自身，并取面积最大的同名窗口以避免命中提示窗；"
              "需要实时画面时先用 SWP_NOACTIVATE 将窗口提到最前（不夺走使用者当前窗口的键盘焦点），"
              "待其重绘后再用 PrintWindow 抓取，因此压在它上方的窗口不会被一起拍进来。",
         size=9.5, color=MUTED)
    para(doc, "本次刻意没有采用整屏截图：抓图时桌面前台是私人聊天窗口，"
              "以及若干与本提交无关的编辑器/终端窗口（打开的是其它项目），"
              "整屏或区域截图会把它们一并交出。四张截图均已逐张目视确认，"
              "窗口内不含密钥与无关项目内容；由于图像无法参与文本层密钥扫描，"
              "本节的检查以人工目视为准。",
         size=9.5, color=MUTED)
