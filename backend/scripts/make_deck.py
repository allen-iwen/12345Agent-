...
def s10_trust(prs):
    ...
    cards = [
        ("System One 决策模型（开源 + 闭源双通道实测）", [
            "laya（Apache 2.0，322M）本地 CPU：647MB / $0 / ~1s",
            "JEV（TypeSafe，OpenRouter alpha）：603ms / $0.00011 每案",
            "同集 18 条：JEV 分类 100%，laya 22.2%（随机 8.3%）",
            "门控 auto/human：JEV 16/0 vs laya 1/15（后者即人工队列）",
            "归因实验排除 CPU：选项改英文后 16.7%→44.4%",
            "定位：判断以规则为主，决策模型补语义 + 门控交人工",
        ]),
        ("双模型交叉复核", [
