...
def s01_cover(prs):
    ...
    textbox(s, 1.1, 4.45, 11, 1.6, [
        ...
        ("实测：事项分类留一验证 100%｜主办单位推荐 94.4%（通用做法仅 5.6%）｜单案 30 秒级（人工基准约 20 分钟）", {"size": 12, "color": TEAL_DK, "space_before": 10}),
    ])
    footer(s)
...
def s10_trust(prs):
    ...
    cards = [
        ("System One 决策模型（Jev）", [
            ...
            "一次调用问完全案 10 个判断（长 state 只发一次）",
            "已实现：契约照官方 OpenAPI、失败自动回退",
        ]),
        ("双模型交叉复核", [
...
def s13_results(prs):
    ...
    textbox(s, 0.7, 5.5, 11.93, 1.2, [
        ("效率对比：单案智能体 30 秒级（含五节点）vs 人工工序基准约 20 分钟；开启双模型复核时为 110–137 秒（可配置）。",
         {"size": 11.5, "color": INK}),
